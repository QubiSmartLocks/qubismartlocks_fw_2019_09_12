...
class FReservaciones extends StatefulWidget {
  FReservaciones({Key key}) : super(key: key);
  static const String ruta = '/reservaciones';
  final v = Vidget();

  @override
  State createState() => _EstadoReservaciones();
}

class _EstadoReservaciones extends State<FReservaciones> {
  GlobalKey<ScaffoldState> _scaffoldKey = GlobalKey<ScaffoldState>();
  List<Reservacion> lista = List<Reservacion>();

  Future<Resultado> getReservaciones() async {
    setState(() {
      widget.v.mensaje = Co.MENSAJE_ESPERA;
    });

    Resultado _resultado = await Reservaciones.todos(lista);
  ...
  Widget build(BuildContext context) {
    widget.v.titulo = RESERVACIONES.ETIQUETA_ENTIDAD;
