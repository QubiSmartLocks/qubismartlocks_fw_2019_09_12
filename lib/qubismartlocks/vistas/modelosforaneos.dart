...
class FModelosForaneos extends StatefulWidget {
  FModelosForaneos({Key key}) : super(key: key);
  static const String ruta = '/modelosforaneos';
  final v = Vidget();

  @override
  State createState() => _EstadoModelosForaneos();
}

class _EstadoModelosForaneos extends State<FModelosForaneos> {
  GlobalKey<ScaffoldState> _scaffoldKey = GlobalKey<ScaffoldState>();
  List<ModeloForaneo> lista = List<ModeloForaneo>();

  Future<Resultado> getModelosForaneos() async {
    setState(() {
      widget.v.mensaje = Co.MENSAJE_ESPERA;
    });

    Resultado _resultado = await ModelosForaneos.todos(lista);
  ...
  Widget build(BuildContext context) {
    widget.v.titulo = MODELOSFORANEOS.ETIQUETA_ENTIDAD;
