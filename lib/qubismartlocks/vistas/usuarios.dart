...
class FUsuarios extends StatefulWidget {
  FUsuarios({Key key}) : super(key: key);
  static const String ruta = '/usuarios';
  final v = Vidget();

  @override
  State createState() => _EstadoUsuarios();
}

class _EstadoUsuarios extends State<FUsuarios> {
  GlobalKey<ScaffoldState> _scaffoldKey = GlobalKey<ScaffoldState>();
  List<Usuario> lista = List<Usuario>();

  Future<Resultado> getUsuarios() async {
    setState(() {
      widget.v.mensaje = Co.MENSAJE_ESPERA;
    });

    Resultado _resultado = await Usuarios.todos(lista);
  ...
  Widget build(BuildContext context) {
    widget.v.titulo = USUARIOS.ETIQUETA_ENTIDAD;
